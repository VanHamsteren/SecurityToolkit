...
async function sendToUrlscan(url, fromQueue = false) {
    ...
    try {
        ...
        if (!apiKey || !visibility) {
            notifyError(
                "URLScan Configuration Missing",
                "Please set your API key and visibility in the extension options.",
            );
            return;
        }
        ...
        if (!response.ok) {
            ...
            const errorMessage =
                ...
                "Unknown error from urlscan.io";
            notifyError(`URLScan API Error (${response.status})`, errorMessage);
        ...
        notifySuccess(
            "URLScan submitted successfully! Opening results soon...",
        );
        ...
        if (data.uuid) {
            ...
            notifyError(
                "Invalid Response",
                "Did not receive a scan UUID from urlscan.io",
            );
        }
    } catch (error) {
    ...
    const checkResults = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Scan not ready yet
                console.log(
                    `URLScan not ready yet (attempt ${attempts}/${MAX_ATTEMPTS})...`,
                );
            ...
            notifyError(
                "Network Error",
                "Failed to check URLScan results. Please check your connection.",
            );
        }
    };
...
function extractDomain(url) {
    try {
        ...
        if (
            ...
            try {
                ...
                if (actualUrl) {
                    ...
                    console.log(
                        `Outlook Safe Link detected. Original: ${url.substring(0, 80)}...`,
                    );
                ...
                console.error(
                    "Failed to extract from Outlook Safe Link:",
                    error,
                );
            }
        }
        ...
        if (url.includes("urlscan.io/domain/")) {
            ...
            if (match && match[1]) {
                console.log(
                    `URLScan.io domain page detected. Extracted: ${match[1]}`,
                );
                return match[1];
            }
        }
 ...
 */
async function addToNextDnsList(
    profileId,
    profileName,
    domain,
    listType,
    ...
    try {
        const { nextdnsApiKey: apiKey } = await browser.storage.sync.get([
            "nextdnsApiKey",
        ]);
        ...
        if (!apiKey) {
            if (!silent) {
                notifyError(
                    "NextDNS Configuration Missing",
                    "Please set your NextDNS API key in the extension options.",
                );
            }
            return;
        }
        ...
        console.log(
            `Calling NextDNS API: POST ${endpoint} with domain: ${domain}`,
        );
        ...
        if (response.ok || response.status === 201 || response.status === 204) {
            const listName =
                listType === "denylist" ? "blocklist" : "allowlist";
            if (!silent) {
                notifySuccess(
                    `NextDNS: Added "${domain}" to ${listName} in profile "${profileName}"`,
                );
            }
            console.log(
                `NextDNS: Successfully added ${domain} to ${listType} in profile ${profileId}`,
            );
        } else {
            const errorText = await response
                .text()
...
async function fetchNextDnsProfiles() {
    try {
        ...
        console.log(
            "Fetching NextDNS profiles with API key length:",
            apiKey.length,
        );
        ...
        if (response.ok) {
            ...
            console.log(
                "NextDNS profiles fetched successfully:",
                profiles.length,
                "profiles",
            );
            ...
            const errorText = await response
                ...
                .catch(() => "No error text");
            console.error(
                "Failed to fetch NextDNS profiles. Status:",
                response.status,
                "Error:",
                errorText,
            );
        ...
        console.error(
            "Exception while fetching NextDNS profiles:",
            error.message,
            error,
        );
        return [];
...
async function getUrlScanQueue() {
    try {
        const { urlscanQueue = [] } = await browser.storage.local.get([
            "urlscanQueue",
        ]);
        return urlscanQueue;
    } catch (error) {
...
async function addToUrlScanQueue(url) {
    try {
        ...
        if (queue.some((item) => item.url === url)) {
            notifyError(
                "Already in Queue",
                `"${url}" is already queued for scanning`,
            );
...
async function processUrlScanQueue() {
    ...
    notifyUser(
        "🚀 Processing Queue",
        `Starting to scan ${pendingItems.length} URLs...`,
    );
    ...
    for (const item of pendingItems) {
        ...
        try {
            console.log(
                `Scanning ${completed + 1}/${pendingItems.length}: ${item.url}`,
            );
            notifyUser(
                "🔍 Scanning",
                `Processing ${completed + 1}/${pendingItems.length}: ${item.url.substring(0, 50)}...`,
            );
    ...
    if (failed === 0) {
        notifySuccess(
            `✓ Queue Complete! Successfully scanned all ${completed} URLs`,
        );
    } else {
        notifyUser(
            "⚠️ Queue Complete",
            `Scanned ${completed} URLs, ${failed} failed`,
        );
    }

    console.log(
        `Queue processing complete: ${completed} success, ${failed} failed`,
    );
}
        ...
        if (nextDnsProfiles.length > 0) {
            ...
            if (nextDnsProfiles.length > 1) {
                try {
                    ...
                    console.log(
                        '  ✓ Created "All Profiles" option for blocklist',
                    );
                    ...
                    console.error(
                        '  ✗ Failed to create "All Profiles" menu:',
                        err,
                    );
                }
            }
            ...
            for (const profile of nextDnsProfiles) {
                try {
                    ...
                    console.log(
                        `  ✓ Created blocklist menu for: ${profile.name || profile.id}`,
                    );
                } catch (err) {
                    console.error(
                        `  ✗ Failed to create blocklist menu for ${profile.id}:`,
                        err,
                    );
                }
            }
            ...
            if (nextDnsProfiles.length > 1) {
                try {
                    ...
                    console.log(
                        '  ✓ Created "All Profiles" option for allowlist',
                    );
            ...
            for (const profile of nextDnsProfiles) {
                try {
                    ...
                    console.log(
                        `  ✓ Created allowlist menu for: ${profile.name || profile.id}`,
                    );
                } catch (err) {
                    console.error(
                        `  ✗ Failed to create allowlist menu for ${profile.id}:`,
                        err,
                    );
                }
            }
            ...
            console.log(
                `✓ Created NextDNS menus for ${nextDnsProfiles.length} profiles`,
            );
        } else {
            console.log(
                "No NextDNS profiles found (API key may not be configured)",
            );
        }
...
browser.contextMenus.onClicked.addListener(async (info) => {
    ...
    // NextDNS - Add to ALL profiles (blocklist)
        ...
        if (successCount === profiles.length) {
            notifySuccess(
                `NextDNS: Added "${domain}" to blocklist in all ${successCount} profiles`,
            );
        } else if (successCount > 0) {
            notifySuccess(
                `NextDNS: Added "${domain}" to ${successCount} profiles (${failCount} failed)`,
            );
        } else {
            notifyError(
                "NextDNS Error",
                `Failed to add "${domain}" to any profiles`,
            );
        }
    }
    ...
    // NextDNS allowlist (single profile)
        ...
        menuItemId.startsWith("nextdns-allowlist-") &&
        menuItemId !== "nextdns-allowlist-all"
    ) {
